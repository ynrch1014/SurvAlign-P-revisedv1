...
## 2. 기존 연구의 한계와 SurvAlign-P로의 필연적 연결 (Motivation & Bridge)

기존 오디오 워터마킹 논문들(AlignMark 등)이 공통적으로 가진 학술적 결함들은, 역설적이게도 본 연구가 **'물리적 사전지식 기반의 에너지 재배치(Survival-Aware Redistribution)'**라는 독특한 구조를 채택하게 된 결정적인 계기가 되었습니다.

### 비판 1: "Bit Accuracy의 착시"와 Exact-Match 달성의 필요성
* **기존 연구의 문제**: 기존 연구들은 "평균 Bit Accuracy > 95%"와 같은 지표로 모델의 우수성을 어필하며 경험적 거리 임계값으로 탐지를 수행합니다. 
* **학술적 오류 분석**: 식별자 데이터베이스가 행성 규모($10^9$)로 커질 경우, 95%의 개별 비트 정확도로는 엉뚱한 사람으로 오인(False Alarm)되는 것을 막을 수 없습니다. 완벽한 식별을 위해서는 **단 1비트의 에러도 허용하지 않는 Exact-Match**가 필수적입니다.
* **본 연구로의 연결 (Why Survival Map?)**: 딥러닝 모델이 오디오 전체 대역에 워터마크를 무작정 흩뿌리는 기존 방식으로는 '모든 비트의 100% 생존'을 절대 보장할 수 없습니다. 이를 해결할 유일한 방법은 **"어떤 공격이 들어와도 절대 무너지지 않을 뼈대 대역"을 사전에 파악하여 그곳에만 에너지를 집중시키는 것**입니다. 이것이 바로 우리가 **물리적 생존율 지도(Survival Map)**라는 사전지식(Prior)을 도입하게 된 논리적 출발점입니다.

### 비판 2: 페이로드 희생(ECC)의 한계와 "Post-hoc 재배치"의 당위성
* **기존 연구의 문제**: 강건성을 올리기 위해 가장 흔히 쓰는 고전적 방법은 에러 정정 코드(ECC)의 도입입니다. 하지만 기존 연구들은 페이로드를 절반으로 깎아먹는 강력한 ECC 적용 결과와 자신들의 딥러닝 모델을 공정하게 비교하지 않았습니다.
* **본 연구로의 연결 (Why Post-hoc Redistribution?)**: 우리는 통신 대역폭(페이로드)을 희생하지 않고도 ECC 이상의 맷집을 확보하고자 했습니다. 이를 위해 기존 워터마킹 모델(AlignMark)을 처음부터 무겁게 재학습하는 비효율을 버리고, 기존 모델이 뱉어낸 잔차(Residual)의 에너지를 가벼운 게이트(Gate)를 통해 **재배치(Redistribution)**하는 **후처리(Post-hoc)** 방식을 고안했습니다. 즉, "에너지를 무식하게 키우거나 용량을 깎는 것"이 아니라 "똑같은 에너지를 영리하게 이사시키는 것"이 본 프레임워크의 존재 이유입니다.
