...
## 2. 베이스라인(AlignMark)의 한계와 SurvAlign-P의 직접적 발전 방향 (Motivation)

본 연구는 최근 SOTA로 발표된 Feature-Aligned 워터마킹(AlignMark, 2606.11828)의 기초 구조를 채택하고 있으나, AlignMark가 가진 '딥러닝 블랙박스'의 본질적 한계를 꼬집으며 이를 극복하는 방향으로 발전했습니다.

### 비판 1: 암시적(Implicit) 데이터 학습 vs 명시적(Explicit) 물리 지식
* **AlignMark의 한계**: AlignMark는 코덱 방어력을 높이기 위해 수많은 공격(EnCodec, 압축 등)을 훈련 파이프라인에 때려 넣고, 딥러닝 모델이 알아서 '안전한 대역'을 찾길 바라는 **암시적(Implicit)이고 데이터 의존적인 방식**을 사용합니다. 이로 인해 에너지가 최적화되지 못하고 파괴될 대역에도 에너지를 낭비하는 비효율이 발생합니다.
* **SurvAlign-P의 발전**: 딥러닝이 알아서 학습하길 기다리지 않습니다. 코덱 공격에 살아남을 확률을 물리적인 수식으로 사전에 계산한 **명시적 지표(Physical Survival Map)**를 도입했습니다. 이를 통해 AlignMark가 불필요하게 낭비하던 워터마크 에너지를 100% 생존 가능한 핵심 뼈대 대역으로 강제 집중(Redistribution)시켜 효율의 극대화를 이뤄냈습니다.

### 비판 2: In-domain 오버피팅 vs Cross-Codec 일반화
* **AlignMark의 한계**: AlignMark는 훈련 때 EnCodec을 맞아가며 맷집을 길렀기 때문에, 평가 시 EnCodec을 잘 방어하는 것이 진정한 일반화 능력인지 아니면 해당 코덱에 대한 오버피팅(Overfitting)인지 증명하기 어렵습니다. 
* **SurvAlign-P의 발전**: 우리는 AlignMark의 구조를 가져오되, 훈련 과정에서 타겟 코덱(FACodec 등)을 완전히 배제합니다. 그럼에도 불구하고 실전에서 AlignMark와 동등하거나 그 이상의 방어력을 발휘합니다. 이는 "코덱을 훈련해서 외운 AlignMark"를 "물리적 지도(Survival Map)만 보고 처음 보는 코덱을 피하는 SurvAlign-P"가 능가했음을 보여주는 완벽한 Cross-Codec Generalization의 증명입니다.

### 비판 3: "Bit Accuracy의 착시"와 Exact-Match 달성의 필요성
* **AlignMark의 한계**: 평균적인 Bit Accuracy 향상에 만족하며 경험적 거리 임계값으로 탐지를 수행합니다. 
* **SurvAlign-P의 발전**: 행성 규모($10^9$)의 식별 시스템에서는 **단 1비트의 에러도 허용하지 않는 Exact-Match**가 필수적입니다. 우리가 고안한 'Post-hoc 에너기 재배치'는 단순한 Bit Accuracy 향상이 아니라, **모든 비트가 동시에 살아남아야 하는 Exact-Match의 가혹한 조건을 달성하기 위한 유일하고 필연적인 아키텍처**입니다.
...
* **수학적 검증 (Fisher's Exact Test & Unpooled Z-test TOST)**: 우리는 실제 오디오 샘플 300개($N=300$)를 대상으로, 1) 임의의 16비트 벡터 공간(Uniform 16-bit)과 2) 256개의 유효 코드워드 공간(ECC Subspace)이 신경망 코덱을 통과할 때의 성공률(P(Hamming <= 2))을 측정했습니다. 이 두 독립된 이항 비율(Binomial Proportions)의 동등성을 검증하기 위해 다음과 같은 2단계 절차를 수행했습니다.
    1. **Fisher's Exact Test**: 초기하분포(Hypergeometric distribution)에 기반한 정확 검정으로, "두 그룹 간의 비율에 유의미한 차이가 존재하는가?"(H0: 차이 없음)를 먼저 확인했습니다. (결과: p-value 0.3523으로 유의미한 차이 없음)
    2. **Two One-Sided Tests (TOST)**: Fisher's Test가 "차이가 없음"을 적극적으로 증명하지 못한다는 맹점을 보완하기 위해, $\pm 15\%p$의 실용적 허용 오차 마진(Margin)을 설정하고 동등성 검정을 수행했습니다. 두 독립 표본의 분산을 모두 고려하는 Unpooled Z-test 기반의 TOST를 적용하여 H0: $|p_1 - p_2| \ge margin$ 을 기각(p_tost = 0.0026)했습니다. (실제 생존 카운트: Uniform 105/300 (35.0%) vs ECC 117/300 (39.0%), `seed=999` 수동 검증 기준)
* **사후 검정력 분석(Power) 확인**: $N=300$ 하에서의 표본 검정력(Post-hoc Power)은 **96.8%**로 산출되었습니다. 즉, 두 그룹 사이에 실제 의미 있는 차이가 존재했다면 이를 잡아낼 확률이 96.8%이며, 차이가 없다고 잘못 결론 내릴 확률(제2종 오류)은 불과 3.2% 미만이라는 뜻입니다. 이처럼 높은 검정력을 확보한 상태에서 **p-value = 0.0026**을 달성하였으므로, **"신경망 코덱은 특정 코드북이나 페이로드 비트값에 종속되지 않는 독립적 에러를 발생시킨다"**는 점이 학술적으로 매우 타당하게 입증됩니다.
